...
$baseline = @((Get-Content (Join-Path $root 'tests\phase03_legacy_baseline.json') -Raw | ConvertFrom-Json) | ForEach-Object { $_.TrimEnd() })
...
$current = @(rg -n --glob '*.gd' --glob '!tests/**' "\b($legacy)\b|change_scene\(" $root | ForEach-Object { $_.Substring($root.Length + 1).TrimEnd() } | Sort-Object)
