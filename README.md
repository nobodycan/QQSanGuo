...
Phase 21 remains in progress. The integration lane now includes a deterministic two-skill by two-enemy CombatAction matrix, a fixed-seed encounter trace spanning target selection, enemy AI transitions, and spawn ownership, a 54,000-tick (15-minute at 60 Hz) component soak, and a 54,000-tick real Steve/Snake adapter-scene soak. Steve auto-chase, hit callbacks, and self-heal now use the shared CombatDriver/Vitals paths; repeated real Snake death now grants money once. The remaining work is the final hard-gate audit.
